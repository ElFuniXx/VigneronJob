...
RMenu.Add('vigne_harvest', 'main', RageUI.CreateMenu("~b~Récolte", "", 10,222))
...
RMenu:Get('vigne_harvest', 'main').Closed = function()
	vigne_harvest = false
	TriggerServerEvent('vuzireee_vignejob:stopCraft')
			...
			Citizen.Wait(1)
				RageUI.IsVisible(RMenu:Get('vigne_harvest', 'main'), true, true, true, function()
					...
					RageUI.ButtonWithStyle("Raisin", "récolter du raisin", {RightLabel = "→"}, true, function(Hovered, Active, Selected) 
						if (Selected) then 
							TriggerServerEvent('vuzireee_vignejob:startHarvest')
					...
					--[[RageUI.ButtonWithStyle("Outil de réparation", "Pour faire des kit de réparation", {RightLabel = "→"}, true, function(Hovered, Active, Selected) 
						if (Selected) then 
							TriggerServerEvent('vuzireee_vignejob:startHarvest2')
						end 
					end)
					RageUI.ButtonWithStyle("Outil de carosserie", "Pour faire des kit de carrosserie", {RightLabel = "→"}, true, function(Hovered, Active, Selected) 
						if (Selected) then 
							TriggerServerEvent('vuzireee_vignejob:startHarvest3')
						end 
					end)]]
				end)
			end
...
RMenu.Add('vigne_craft', 'main', RageUI.CreateMenu("~b~Traitement", "", 10,222))
			...
			Citizen.Wait(1)
				RageUI.IsVisible(RMenu:Get('vigne_craft', 'main'), true, true, true, function()
					...
					--[[RageUI.ButtonWithStyle("Faire des Grand crus", "Il vous faut du raisins et du vin", {RightLabel = "→"}, true, function(Hovered, Active, Selected) 
						if (Selected) then 
							TriggerServerEvent('vuzireee_vignejob:startCraft')
						end 
					end)
					RageUI.ButtonWithStyle("Vin", "Il vous faut du raisin pour fabriquer une bouteille.", {RightLabel = "→"}, true, function(Hovered, Active, Selected) 
						if (Selected) then 
							TriggerServerEvent('vuzireee_vignejob:startCraft2')
						end 
					end)]]
					RageUI.ButtonWithStyle("Jus de raisin", "Il vous faut du raisin pour fabriquer un jus", {RightLabel = "→"}, true, function(Hovered, Active, Selected) 
						if (Selected) then 
							TriggerServerEvent('vuzireee_vignejob:startCraft3')
						end 
					end)
				end)
			end
...
RMenu.Add('vigne_sell', 'main', RageUI.CreateMenu("~b~Vente", "", 10,222))
RMenu:Get('vigne_sell', 'main'):SetSubtitle("~r~ la moula")
RMenu:Get('vigne_sell', 'main').EnableMouse = false
RMenu:Get('vigne_sell', 'main').Closed = function()
	vigne_sell = false
	TriggerServerEvent('vuzireee_vigne:stopSell')
	FreezeEntityPosition(playerPed, false)
end
		...
		while vigne_sell do
			Citizen.Wait(1)
				RageUI.IsVisible(RMenu:Get('vigne_sell', 'main'), true, true, true, function()
					FreezeEntityPosition(playerPed, true)
					RageUI.ButtonWithStyle("Vente de Jus de Raisin", "Récupère la moula", {RightLabel = "→"}, true, function(Hovered, Active, Selected) 
						if (Selected) then 
							TriggerServerEvent('vuzireee_vigne:startSell')
						end 
					end)
				end)
			end
...
RMenu.Add('vigne_menu', 'main', RageUI.CreateMenu("~b~Vigneron", "", 10,222))
            ...
            Citizen.CreateThread(function()
                        ...
                        RageUI.ButtonWithStyle("Factures", "~r~Faire une facture.", {RightLabel = "→→"}, true, function(Hovered, Active, Selected) 
                            if (Selected) then
                                ...
                                if closestPlayer == -1 or closestDistance > 3.0 then
                                    TriggerEvent("vuzireee:showAdvancedNotification", '~r~Facture', '~r~Benny\'s', '~r~Il n\'y a personne autour de vous.', 'CHAR_BLOCKED', 'spawn', 8)
                                else
                                ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
                                    ...
                                    if amount == nil then
                                        ...
                                        if closestPlayer == -1 or closestDistance > 3.0 then
                                            ...
                                            TriggerServerEvent('vuzireee_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_vigne', 'Benny\'s', amount)
                ...
                RageUI.IsVisible(RMenu:Get('vigne_menu', 'poing'), true, true, true, function()
                    RageUI.ButtonWithStyle("Analyse Moteur", "~b~Analyse du moteur pour voir si il faut faire une réparation ou pas.", {RightLabel = "~r~200~w~$"}, true, function(Hovered, Active, Selected) 
                        if (Selected) then 
                            TriggerServerEvent('vuzireee_billing:sendBillBill', GetPlayerServerId(closestPlayer), 'society_vigne', '~b~LS\'Custom : Analyse Moteur', '200')
                    ...
                    RageUI.ButtonWithStyle("Controle technique", "~b~Analyse du véhicule entier", {RightLabel = "~r~500~w~$"}, true, function(Hovered, Active, Selected) 
                        if (Selected) then 
                            TriggerServerEvent('vuzireee_billing:sendBillBill', GetPlayerServerId(closestPlayer), 'society_vigne', '~b~LS\'Custom : Controle technique', '500')
							ESX.ShowNotification('~b~Vous avez envoyé une facture.')
                        end 
                    end)
                ...
                RageUI.IsVisible(RMenu:Get('vigne_menu', 'repa'), true, true, true, function()
					RageUI.ButtonWithStyle("Vitre", "~b~Facture pour une/des vitre(s) cassée(s)", {RightLabel = "~r~200~w~$"}, true, function(Hovered, Active, Selected) 
						if (Selected) then 
                            TriggerServerEvent('vuzireee_billing:sendBillBill', GetPlayerServerId(closestPlayer), 'society_vigne', '~b~LS\'Custom : Vitre', '800')
							ESX.ShowNotification('~b~Vous avez envoyé une facture.')
						end 
					end)
					RageUI.ButtonWithStyle("Carosserie", "~b~Réparation de la carrosserie", {RightLabel = "~r~200~w~$"}, true, function(Hovered, Active, Selected) 
						if (Selected) then 
                            TriggerServerEvent('vuzireee_billing:sendBillBill', GetPlayerServerId(closestPlayer), 'society_vigne', '~b~LS\'Custom : Carosserie', '800')
							ESX.ShowNotification('~b~Vous avez envoyé une facture.')
						end 
					end)
				end, function()
								...
								RageUI.ButtonWithStyle(infos.label,nil, {RightBadge = RageUI.BadgeStyle.Car}, ESX.PlayerData.job.grade >= infos.minimum_grade, function(_,_,s)
									if s then
										Citizen.CreateThread(function()
											...
											TriggerEvent("vuzireee:showAdvancedNotification", '~b~Vigneron', '~g~Votre véhicule de service a été sorti.', '~b~Voici la plaque : ~r~'..GetVehicleNumberPlateText(vehicle).. '\n~g~Vous avez aussi reçu les clés !', 'CHAR_LS_CUSTOMS', 'spawn', 8)
